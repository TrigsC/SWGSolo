...
function HologrindJediManager:useItem(pSceneObject, itemType, pPlayer)
	...
	if itemType == ITEMHOLOCRON then
		...
		--HologrindJediManagerHolocron.useHolocron(pSceneObject, pPlayer)
	end
...
function HologrindJediManager:registerObservers(pCreatureObject)
    --dropObserver(BADGEAWARDED, "HologrindJediManager", "badgeAwardedEventHandler", pCreatureObject)
...
function HologrindJediManager:onPlayerLoggedIn(pPlayer)
	...
	--if (HologrindJediManagerCommon.isVillageEligible(pPlayer) and not CreatureObject(pPlayer):hasSkill("force_title_jedi_novice")) then
...
function HologrindJediManager:onPlayerLoggedOut(pPlayer)
	if (pPlayer == nil) then
		return
	end
...
function HologrindJediManager:onFSTreeCompleted(pPlayer, branch)
	if (pPlayer == nil) then
		return
	end

	--if (QuestManager.hasCompletedQuest(pPlayer, QuestManager.quests.OLD_MAN_FINAL) or HologrindJediManagerCommon.hasJediProgressionScreenPlayState(pPlayer, VILLAGE_JEDI_PROGRESSION_COMPLETED_VILLAGE) or HologrindJediManagerCommon.hasJediProgressionScreenPlayState(pPlayer, VILLAGE_JEDI_PROGRESSION_DEFEATED_MELLIACHAE)) then
	--	return
	--end
	--if (HologrindJediManagerCommon.getLearnedForceSensitiveBranches(pPlayer) >= NUMBEROFTREESTOMASTER) then
	--	HologrindJediManagerCommon.setJediProgressionScreenPlayState(pPlayer, VILLAGE_JEDI_PROGRESSION_COMPLETED_VILLAGE)
	--	FsOutro:startOldMan(pPlayer)
	--end
end

function HologrindJediManager:onSkillRevoked(pPlayer, pSkill)
	if (pPlayer == nil) then
		return
	end
